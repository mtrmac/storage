...
type roLayerBigDataStore interface {
	// SetBigData stores a (potentially large) piece of data associated
	// with this ID.
	bigData(token layerReadToken, id, key string) (io.ReadCloser, error)

	// bigDataNames() returns a list of the names of previously-stored pieces of
	// data.
	bigDataNames(token layerReadToken, id string) ([]string, error)
}
...
// void a type returned when we don’t care, used to make generic wrappers a bit easier to use.
// Declare a function returning void, and return a void{} value.
type void struct{}

// readAllLayerStores processes allLayerStores() in order:
// It locks the store for reading, checks for updates, and calls
//
//	(data, done, err) := fn(store, token)
//
...
//		return res, err
//	}
func readAllLayerStores[T any](s *store, fn func(store roLayerStore, token layerReadToken) (T, bool, error)) (T, bool, error) {
	...
	for _, s := range layerStores {
		store := s
		if res, done, err := layerReadAccess(store, func(token layerReadToken) (T, bool, error) {
			return fn(store, token)
		}); done {
			return res, true, err
		}
	}
...
func (s *store) writeToAllStores(fn func(rlstore rwLayerStore, layerToken layerWriteToken) error) error {
	rlstore, err := s.getLayerStore()
	if err != nil {
		return err
	}

	return layerWriteAccess(rlstore, func(layerToken layerWriteToken) error {
		if err := s.imageStore.startWriting(); err != nil {
			return err
		}
		defer s.imageStore.stopWriting()
...
func (s *store) Metadata(id string) (string, error) {
	if res, done, err := readAllLayerStores(s, func(store roLayerStore, token layerReadToken) (string, bool, error) {
		if store.exists(token, id) {
			res, err := store.metadata(token, id)
			return res, true, err
		}
		return "", false, nil
	}); done {
...
func (s *store) LayerBigData(id, key string) (io.ReadCloser, error) {
	foundLayer := false
	if res, done, err := readAllLayerStores(s, func(store roLayerStore, token layerReadToken) (io.ReadCloser, bool, error) {
		data, err := store.bigData(token, id, key)
		if err == nil {
			return data, true, nil
		}
		if store.exists(token, id) {
...
func (s *store) SetContainerBigData(id, key string, data []byte) error {
	_, err := writeToContainerStore(s, func() (void, error) {
		return void{}, s.containerStore.SetBigData(id, key, data)
	})
	return err
}
...
func (s *store) Exists(id string) bool {
	...
	if res, done, _ := readAllImageStores(s, func(store roImageStore) (bool, bool, error) {
		if store.Exists(id) {
			return true, true, nil
		}
		return false, false, nil
...
func (s *store) DeleteImage(id string, commit bool) (layers []string, err error) {
	...
	if err := s.writeToAllStores(func(rlstore rwLayerStore, layerToken layerWriteToken) error {
		if s.imageStore.Exists(id) {
...
func (s *store) Images() ([]Image, error) {
	var images []Image
	if _, _, err := readAllImageStores(s, func(store roImageStore) (void, bool, error) {
...
func (s *store) GarbageCollect() error {
	_, firstErr := writeToContainerStore(s, func() (void, error) {
